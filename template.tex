...
\usepackage{ragged2e} %更好的斷行、對齊(\Centering, \RaggedLeft, and \RaggedRight \justifying)


\usepackage{hyperref} % 生成書籤
\usepackage{bookmark} % 書籤加強版
\usepackage[square, comma, numbers, super, sort&compress]{natbib}  % bib cite加強版
...
\usepackage[dvipsnames, svgnames, x11names, usenames]{xcolor}% 可以使用顏色\color{red}
...
\usepackage{multirow} % 表格欄位合併
\usepackage{diagbox} % 表格畫斜線，上下有字

\usepackage{enumerate}  % 有序清單
\usepackage{enumitem} % 無序清單
\usepackage{listings} % 清單(coding)
\usepackage{zhnumber} % 數字變中文、中文編號 https://ctan.math.utah.edu/ctan/tex-archive/macros/latex/contrib/zhnumber/zhnumber.pdf

\usepackage{forest} %樹狀圖
...
\usepackage{verbatim}  % 原樣輸出文本，忽视TeX命令。
\usepackage{cancel} % 數學環境內把字劃掉 \cancel, \bcancel \xcancel \cancelto{<value>}{<expression>}
\usepackage[normalem]{ulem} % 刪除線\sout{}
\usepackage{pifont} %加入一些符號 https://dl.icdst.org/pdfs/files3/1900293ae8829c4c53fe55f2c9f0077c.pdf
\usepackage{bbm} % 填補 mathbb 沒有小寫的缺憾 ex. $\mathbbm{1}$
...
\newrefformat{dsc}{Discussion\ref{#1}} %討論
...
\newcount\thisyear%%%中華民國年月日
\thisyear=\year%%%中華民國年月日
\advance\thisyear by -1911%%%中華民國年月日
\newcommand{\ROCtoday}{\the\thisyear 年 \the\month 月 \the\day 日}%%%中華民國年月日
